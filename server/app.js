...
const server = app.listen(3000, () => { //listen()연결요청 대기
...
fs.watchFile(__dirname + '/sql.js', (curr, prev) => {
  // file 레파지토리를 감시하다가 변경되는 것을 감지. 저장소(Git repository)란 말그대로 파일이나 폴더를 저장해 두는 곳
...
const db = { //데이터베이스 연동
  database: "eunk",
  connectionLimit: 10, //connectionLimit : 최대 커넥션 개수, 기본 10개. 데이터베이스갯수.
  host: "127.0.0.1",
  user: "root",
  password: "1234"
};
...
app.post("/api/kakaoLogin", async (request, res) => {
  ...
  try {
    ...
    if (request.body.param.length > 0) {
      ...
      res.send(request.body.param[0]); // 받아왔던 파라미터를 보내줌.  result의 약자로
    } else {
      // 파라미터 없이 api를 호출했을 시
      res.send({
...
app.post('/api/logout', async (request, res) => {
  ...
  request.session.destroy(); //Destroy 함수를 호출하여 Object를 제거하면 즉시 제거되는 것이 아니라 해당 Frame의 마지막에 제거
  res.send('ok');
});

app.post('/upload/:productId/:type/:fileName', async (request, res) => { //요청한 결과 값
  // file 업로드 api
  let {
    productId,
    type,
    fileName
  } = request.params; // 받아온 파라미터
  ...
  if (!request.body.data) return fs.unlink(file, async (err) => res.send({
    ...
    //unlink()함수는 시스템 호출을 사용하여 파일에 대한 디렉토리 항목을 지우고 링크 개수를 감소시킵니다. (반환값: 0 성공, -1 에러) unlink를 사용하려면 파일의 디렉토리 항목에 포함된 모든 디렉토리에 대해 쓰기와 실행 권한이 필요 함.
    err
  }));
  const data = request.body.data.slice(request.body.data.indexOf(';base64,') + 8); // body에서 받아온 data를 base64를 통해 인코딩. uploads폴더/8개제품
  //Base64를 사용하는 가장 큰 이유는 Binary 데이터를 텍스트 기반 규격으로 다룰 수 있기 때문이다. JSON과 같은 문자열 기반 데이터 안에 이미지 파일등을 Web에서 필요로 할때 Base64로 인코딩하면 UTF-8과 호환 가능한 문자열을 얻을 수 있다.
  ...
  fs.writeFile(file, data, 'base64', async (error) => { // file 경로로 data를 base64기반으로 wirteFile
    await req.db('productImageInsert', [{ // sql productImageInsert를 통해 받아온 파라미터를 Insert
...
const req = {
  ...
  async db(alias, param = [], where = '') {
    return new Promise((resolve, reject) => dbPool.query(sql[alias].query + where, param, (error, rows) => {
      if (error) {
        if (error.code != 'ER_DUP_ENTRY') // 에러 시
          console.log(error);
        resolve({
          error
        });
